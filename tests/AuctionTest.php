...
class AuctionTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $startTime = time() + 3600;
        $endTime = time() + 3600 * 2;
        $this->auction = new \ToBeAgile\Auction(
            $this->user,
            $startPrice,
            $itemDescription,
            $startTime,
            $endTime,
            \ToBeAgile\Auction::CATEGORY_OTHER
        );
    ...
    public function testCloseBidder()
    {
        ...
        $this->auction->onClose();
        $this->assertTrue(
            $this->postOffice->doesLogContain(
                $this->auction->getUser()->getUserEmail(), $this->auction->getSoldMessage()
            )
        );
        $this->assertTrue(
            $this->postOffice->doesLogContain(
                $this->auction->getHighestBidder()->getUserEmail(), $this->auction->getWonMessage()
            )
        );
    }
    ...
    public function testCloseNoBidder()
    {
        $this->assertFalse(
            $this->postOffice->doesLogContain(
                $this->auction->getUser()->getUserEmail(), $this->auction->getNoBidsMessage()
            )
        );
        $this->auction->onStart();
        $this->auction->onClose();
    ...
    public function testLuxuryCar()
    {
        ...
        $this->assertEquals(
            $auction->getHighestBid() *
                (1 + \ToBeAgile\Process\LuxuryCarTaxFee::TAX_RATE) +
                \ToBeAgile\Process\CarShippingFee::FEE,
            $auction->getBuyerAmount(),
            '',
            0.01
        );
    ...
    public function testPreferredSellerReducedRate()
    {
        ...
        $this->auction->onClose();
        $this->assertEquals(
            $amount * (1 - \ToBeAgile\Process\SellerFee::PREFERRED_FEE_RATE),
            $this->auction->getSellerAmount(),
            '',
            0.01
        );
    }
